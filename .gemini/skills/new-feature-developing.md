...
2.  **(2) Appearance Designing:** 디자인 및 UI/UX 스타일링 (ux-designer-subagent 활용)
3.  **(3) Implementation Planning:** 구현 계획 수립 (frontend-developer-subagent 활용)
4.  **(4) Implementation:** 기능 구현 및 코드 작성 (frontend-developer-subagent 활용)
5.  **(5) QA Check:** 품질 검증 및 미해결 이슈 관리 (qa-engineer-subagent 활용)
6.  **(6) Documentation:** 최종 문서화 및 히스토리 정리 (product-owner-subagent 활용)
...
- **구현 가능성 검토:**
  ...
  - `product-owner-subagent`는 이 결과를 검토하여 필요 시 `product-owner-subagent` 문서를 업데이트합니다. 기획 자체의 변경이 필요하면 (1)단계로 회귀합니다.
- **전환:** 디자인 확정 시 (3)단계 진행 여부를 묻고, Yes 시 진행합니다. No 시 (2)단계를 다시 시작합니다.

### (3) Implementation Planning
- **에이전트:** `@frontend-developer-subagent` 호출
- **동작:** `product-owner-subagent` 및 `ux-designer-subagent` 결과물을 바탕으로 구현 계획을 세워 `.gemini/skills/new-feature-developing/history/{feature명}/implementation-planning/{번호}.md`에 기록합니다. {feature명}은 kebab-case를 따릅니다.
...
### (4) Implementation
- **에이전트:** `@frontend-developer-subagent` 호출
- **동작:** 계획에 따라 코드를 작성하고 기능을 개발합니다. 사용자 피드백을 실시간 반영하여 최적화합니다.
- **기록:** 개발 내용은 `.gemini/skills/new-feature-developing/history/{feature명}/implementation/{번호}.md`에 기록합니다.
- **전환:** 개발 완료 후 (5)단계 진행 여부를 묻습니다. No 시 사용자가 원하는 Phase(1, 2, 3 중 하나)로 회귀합니다.

### (5) QA Check
- **에이전트:** `@qa-engineer-subagent` 호출
- **검증 힌트:** `product-owner-subagent`, `ux-designer-subagent`, `frontend-developer-subagent` 단계의 결과물들을 참고하여 검증합니다.
...
- **회귀 로직:** 검증 실패 시 각 단계별 수정 사항을 정리하여 제안하고 사용자 수용 시 해당 Phase로 돌아가 수정을 진행합니다. 거절 시 (6)단계로 넘어갑니다.
- **전환:** 검증 완료 후 (6)단계 진행 여부를 묻습니다. No 시 사용자가 원하는 Phase(1~5 중 하나)로 회귀합니다.

### (6) Documentation
- **에이전트:** `@product-owner-subagent` 호출
- **동작:** 모든 히스토리 문서(`product-owner-subagent`, `ux-designer-subagent`, `frontend-developer-subagent`, `qa-engineer-subagent`, Unresolved Issues)를 종합하여 `docs/features/{feature명}/00.feature-overview.md`를 작성합니다.
- **명시 사항:** 미해결 이슈 존재 시 관련 파일 경로를 명시하고, 개발 내역 히스토리가 `.gemini/skills/new-feature-developing/**` 내에 있음을 기록하여 추적 가능하게 합니다.
- **종료:** 완료 후 사용자에게 **'종료'** 또는 **'계속'** 여부를 묻습니다. '계속' 선택 시 돌아가고 싶은 Phase(1~6 중 하나)를 선택받아 이동합니다.
